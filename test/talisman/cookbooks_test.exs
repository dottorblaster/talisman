defmodule Talisman.CookbooksTest do
  ...
  describe "Cookbooks context" do
    ...
    test "create a cookbook, append a recipe to it, with an empty category, then edit it" do
      %{author_uuid: author_uuid} =
        new_cookbook = %{author_uuid: Faker.UUID.v4(), name: Faker.Pokemon.name(), recipes: []}
    ...
    test "create a cookbook, append a recipe to it, with an empty category, then delete it" do
      ...
      Cookbooks.delete_recipe(author_uuid, %{
        cookbook_uuid: cookbook_uuid,
        recipe_uuid: recipe_uuid
      })

      assert_raise(Ecto.NoResultsError, fn ->
        Cookbooks.get_recipe!(recipe_uuid)
      end)
    end
  end
end
